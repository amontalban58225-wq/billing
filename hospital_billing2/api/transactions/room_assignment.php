        ...
        try {
            $stmt = $this->conn->query("
                SELECT ra.assignmentid,
                       ...
                       CONCAT(
                           IFNULL(p.lastname, ''),
                           IF(p.lastname IS NOT NULL AND p.firstname IS NOT NULL, ', ', ''),
                           IFNULL(p.firstname, ''),
                           ' ',
                           IFNULL(p.middlename, ''),
                           ' ',
                       ...
                       IFNULL(r.status, 'Unknown') AS room_status,
            ...
            foreach ($results as &$row) {
                $row['patient_name'] = trim($row['patient_name']) !== '' ? $row['patient_name'] : 'Unknown';
                $row['room_status'] = $row['room_status'] ?: 'Unknown';
